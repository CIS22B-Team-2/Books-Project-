...
void Database::sortByRetailPrice()
{
	int startScan, minIndex;
	Book minBook;
	for (startScan = 0; startScan < (numBooks - 1); startScan++)
...
void Database::sortByWholesalePrice()
{
	int startScan, minIndex;
	Book minBook;
	for (startScan = 0; startScan < (numBooks - 1); startScan++)
...
int Database::searchByTitle(std::string title)
{
...
int Database::searchByISBN(std::string isbn)
{
	int first = 0, last = numBooks - 1, middle, position = -1;
	bool found = false;
	while (!found && first <= last)
...
//}
//
//int SearchByName(Book* book, int size, std::string name)
//{
//	int position = -1;
